...
class CubicPolynomialTrajectoryGenerator:
    ...
    def _generate_time_nodes(self):
        """生成每个路径点对应的时间节点（按路径点距离分配时间，保证运动平滑）"""
        # 计算路径点之间的欧几里得距离
        distances = [0.0]
    ...
    def _cubic_polynomial_interpolation(self, x_nodes, t_nodes, t_trajectory):
        """三次多项式插值（生成单轴光滑轨迹）"""
        # 自主实现三次插值（也可使用scipy的插值工具，符合题目要求）
    ...
    def generate_trajectory(self, time_resolution=0.01):
        ...
        return self.trajectory_t, np.vstack((self.trajectory_x, self.trajectory_y, self.trajectory_z)).T
    
    def plot_trajectory(self):
        """可视化轨迹（包含x、y、z三轴时间历史，及离散路径点）"""
